...
## Day 15 (12:00) — Time-Period Comparison
Business analysts can say "compare 2023 vs 2024", "Q1 vs Q2 performance", "how did this year compare to last?" and receive a `TimeWindowCard` inline in chat showing side-by-side metrics for two date ranges. `compare_time_windows()` in `core/analyzer.py`; `GET /api/data/{id}/compare-time-windows`; `_TIMEWINDOW_PATTERNS` + `_detect_timewindow_request()` in chat.py → `{type:"time_window_comparison"}` SSE event; `TimeWindowCard` (side-by-side table, green/red arrows, notable changes highlighted).
